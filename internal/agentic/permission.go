...
const (
	...
	// GateAsk pauses for the user (permission popover).
	GateAsk
...
// The user chose the protective scope: EVERY risky/destructive command gets a
// native gate, not just out-of-repo ones.
...
func AllowKey(cmd string) string {
	...
	// Strip leading env assignments (FOO=bar cmd …) and redirections so the
	// first real word is inspected.
...
func GateCommand(cmd, repoRoot string, allow map[string]bool) GateDecision {
	...
	if allow != nil {
		...
		// command scoping (Claude's Bash(npm *)), audit P3.
